...
| Screen | What you see |
|--------|-------------|
| **Splash** | Network loader — animated dots while questions are fetched |
...
- 🔥 **Streak celebration** — streak ≥ 3 triggers a Lottie fire overlay, **auto-dismissed by the ViewModel** at 1.5 s; best streak persists across restarts
- ✅ **Instant feedback banner** — Correct / Not quite! / Skipped
...
```
app/
├── ui/   MainActivity.kt · QuizViewModelFactory.kt
└── di/   AppComponent.kt · ViewModelModule.kt

data/
├── domain/model/   Question.kt · QuizResult.kt
├── infra/api/      QuestionApiService.kt  (mock, 1 s delay)
├── infra/dto/      QuestionDto.kt
├── infra/mapper/   QuestionDtoMapper.kt
├── infra/repository/ QuestionRepository.kt · QuestionRepositoryImpl.kt
└── di/             DataModule.kt

feature/quiz/
├── domain/contract/  QuizAppContract.kt
├── domain/state/     QuizUiState.kt
├── app/              QuizAppService.kt
├── di/               QuizModule.kt
└── ui/
    ├── viewmodel/    QuizViewModel.kt
    ├── screen/       NetworkLoaderScreen · QuizScreen · ResultScreen · QuizEntryPoint
    ├── component/    OptionCard · ProgressBar · StreakBadge · StreakCelebrationOverlay · TopBar
    └── theme/        QuizAppTheme · QuizColor · QuizFont
```
...
1. Open the `MCQQuizApp` folder in **Android Studio Meerkat+**
